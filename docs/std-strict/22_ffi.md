...
- 解析（Parser）：`[~]` 部分完成  
  已覆盖：`extern fn` 声明、`name: type` 形参风格、`...` 变参标记、返回类型基础解析、链接名属性 `#[name = "Symbol"]`（用于源码名与链接符号不一致）。  
  未覆盖：与文档完全一致的全部 FFI 语法分支与调用细节。
- 语义（Semantic）：`[ ]` 未开始（跨 ABI 的参数分类/变参规则尚未落地）
- 编译（Windows x86_64）：`[~]` 部分完成（x86 后端按 `ExternFnDecl` 形参与类型走 **Win64 通用装参**（`rcx/rdx/r8/r9` + 第 5 实参 `rsp+32`），再用 `#[name = "..."]` 发射 `call`；不再为 `ExitProcess`/`GetStdHandle`/`WriteFile` 等维护手写名字分支。变参 `extern` 仍不支持。）
...
源码里的 `extern fn` 名字可以按可读性来取（例如蛇形 `write_file`），而链接器里的符号仍是平台 ABI 里的名字（例如 Windows 的 `WriteFile`）。在声明前加上属性：
...
调用时仍使用**声明名**（如 `os::write_file(...)`）；编译器根据 `#[name]` 发射对链接符号的 `call`。

当前实现约定：

- 属性名必须是 `name`，值为**字符串字面量**（双引号）。
- 未写 `#[name = ...]` 时，链接符号默认与 `extern fn` 的声明名一致。
...
`extern fn` 的形参名字**等价于**用户函数里的寄存器别名（见 [05. 寄存器别名](./05_registers.md#寄存器别名)）——想用 `@` 形式显式写出来也可以（`fmt @ rdi: *u8`），但 C ABI 的槽绑定由平台决定而非用户，所以通常只写形参名更干净。
