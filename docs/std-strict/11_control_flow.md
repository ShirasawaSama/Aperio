...
Std-Strict 保留了“标签 + 跳转”这套汇编原生控制流，同时提供轻量结构化 `if` 与 `save` 语句来提升可读性。

如果你想要结构化控制流，使用 Loose 模式（`.apo`），它会在编译时被降级为本章描述的形式。

> 兼容目录说明：`09_control_flow.md` 为导航页，本章是控制流唯一规范正文。  
> 函数调用参数规则 A 在 [12. 函数 - 调用函数](./12_functions.md#调用函数)。

## 章节实现状态

- 解析（Parser）：`[~]` 部分完成  
  已覆盖：参数化标签（`@label(r1: u32):`）、`goto(@label(...))`、`if (...) goto(...)`、结构化 `if (...) { ... } else { ... }`、`save (...) { ... }`、`return`、赋值与多目标赋值。  
  已接入：文档代码块采样 parse 回归（05/11/12 基线）。  
  未覆盖：更复杂条件组合规则与全部边界错误场景。
- 语义（Semantic）：`[~]` 部分完成  
  已覆盖：禁止向内跳（不可从外层跳进 `if/save` 内部标签）、label 参数个数检查、label 参数类型重建、`save` 块“自然执行到块尾”时的类型快照恢复、`if/else` 合流类型冲突检查、同一 label 多前驱入边参数类型一致性检查、`goto` 与自然落入（fallthrough）到同一 label 的入边状态一致性检查（按 label 前驱集合统一校验）。  
  未覆盖：完整 CFG 级别控制流合流类型一致性与更细粒度跨路径检查。
- 编译（Windows x86_64）：`[~]` 部分完成（`goto`/`if-goto` 最小分支发射已接通，`save` 已在 lowering 层降级为显式保存/恢复赋值序列）
...
`goto` 只允许跳到**同级或外层**可见的标签，不允许“跳进”更内层语句块（例如 `if` 块、`save` 块）：

- 允许：块内跳到块外标签（向外跳）
- 允许：同一层级标签之间互跳
- 禁止：从块外跳入块内标签（向内跳）

这样做的目标是保证控制流可分析性，避免绕过块入口处的参数绑定、类型重置或保存恢复语义。

```rust
@entry:
if (cond) goto(@inside)          // 非法：从外层跳进 if 内部
goto(@after)                     // 合法

if (cond) {
@inside:
    goto(@after)                 // 合法：从内层跳到外层
}
...
除了 `if (...) goto(...)`，Std-Strict 也支持结构化块写法：
...
结构化 `if` 只是语法层增强，语义上仍受本章的跳转边界和类型流规则约束。
...
在语义层，`save` 只有在**自然执行到块尾**时，才会恢复进入块前的寄存器类型快照。  
如果在块内通过 `goto` / `return` 提前离开，则该路径**不会触发恢复**。
